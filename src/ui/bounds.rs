...
pub fn get_children(
    ...
    let Ok((_, _, _, children)) = query.get(current) else {
        return;
    };
...
pub fn get_global_bounds(
    ...
    let Ok((_, global_bounds, local_bounds, parent)) = global_bounds_query.get(current) else {
        return None;
    };

    if let Some(parent) = parent {
        let bounds = get_global_bounds(**parent, global_bounds_query, acsii_cam_query)
