...
use super::{bounds::{AsciiBounds, AsciiGlobalBounds}, HorizontalAlignment, Padding, VerticalAlignment};
...
fn mark_positions_dirty(
    mut changed_bounds : Query<(Entity, &mut AsciiGlobalBounds, Ref<AsciiPosition>, Option<&Children>)>
) {
    ...
    for entity in entities {
        ...
        children.into_iter().for_each(|child| { dirty.insert(child); });
        dirty.insert(entity);
    }
...
fn get_children(
    ...
    query: &Query<(Entity, &mut AsciiGlobalBounds, Ref<AsciiPosition>, Option<&Children>)>
) {
    let Ok((_, global_bounds, _, children)) = query.get(current) else { return };
...
fn update_positions(
    mut bounded_entities : Query<(Entity, &mut AsciiGlobalBounds, Option<&AsciiPosition>, Option<&Parent>)>,
...
fn get_global_bounds (
    current : Entity,
    global_bounds_query : &Query<(Entity, &mut AsciiGlobalBounds, Option<&AsciiPosition>, Option<&Parent>)>,
    acsii_cam_query : &Query<&AsciiCamera>
) -> Option<AsciiBounds> {
    ...
    let Ok((_, global_bounds, local_position, parent)) = 
        global_bounds_query.get(current) else { return None; };
    
    if let Some(parent) = parent {
        let parent_bounds = get_global_bounds(**parent, global_bounds_query, acsii_cam_query).unwrap_or(global_bounds.bounds.clone());
...
impl AsciiPosition {
    ...
    pub fn relavtive(x : i32, y : i32, width : u32, height : u32) -> Self {
        AsciiPosition::Relative { bounds : AsciiBounds::new(x, y, width, height) }
    }
    
    pub fn align(width : u32, height : u32, horizontal: HorizontalAlignment, vertical: VerticalAlignment) -> Self {
        AsciiPosition::Aligned { width, height, horizontal, vertical }
    }
    
    pub fn format_bounds(&self, parent_bounds : &mut AsciiBounds, child_bounds : &mut AsciiBounds) {
        match self {
            AsciiPosition::Aligned { width, height, horizontal, vertical } => {
                Self::format_bounds_aligned(*width, *height, *horizontal, *vertical, parent_bounds, child_bounds)
            },
            AsciiPosition::Padded { padding } => {
                Self::format_bounds_padded( *padding, parent_bounds, child_bounds)
            },
            AsciiPosition::VerticalSlice { total_silces, slice } => {
                // Self::format_bounds_vertical_slice(*total_silces, *slice, parent_bounds)
            },
            AsciiPosition::HorizontalSlice { total_silces, slice } => {
    ...
    pub fn create_bounds_aligned(
        ...
        Self::format_bounds_aligned(width, height, horizontal_alignment, vertical_alignment, parent_bounds, &mut child);
        child
    }
    ...
    pub fn format_bounds_aligned(
        ...
        let x = match horizontal_alignment {
            ...
            HorizontalAlignment::Center => ((parent_bounds.width as f32 / 2.0) - (width as f32 / 2.0)).floor().max(0.0) as i32,
            HorizontalAlignment::Right => parent_bounds.width.saturating_sub(width) as i32,
        };
        let y = match vertical_alignment {
            VerticalAlignment::Top => 0,
            VerticalAlignment::Center => ((parent_bounds.height as f32 / 2.0) - (height as f32 / 2.0)).floor().max(0.0) as i32,
    ...
    fn format_bounds_padded(padding : impl Into<Padding>, parent_bounds: &AsciiBounds, child_bounds : &mut AsciiBounds) {
        let padding = padding.into();
        let horizontal_difference = parent_bounds.width as i32 - padding.left as i32 - padding.right as i32;
        let vertical_difference = parent_bounds.height as i32 - padding.top as i32 - padding.bottom as i32;
    ...
    fn format_bounds_vertical_slice(total_silces: u32, slice: u32, parent_bounds: &AsciiBounds, child_bounds : &mut AsciiBounds) {
        todo!()
    }

    fn format_bounds_horizontal_slice(total_silces: u32, slice: u32, parent_bounds: &AsciiBounds, child_bounds : &mut AsciiBounds) {
        todo!()
    }
    
    fn format_bounds_relative(bounds : &AsciiBounds, parent_bounds: &AsciiBounds, child_bounds : &mut AsciiBounds) {
